...
BEGIN_MESSAGE_MAP(CDlg_t1601, CDialog)
	...
	ON_MESSAGE	 ( WM_USER + XM_RECEIVE_DATA, OnXMReceive1601	  )
	ON_MESSAGE	 ( WM_USER + XM_TIMEOUT_DATA, OnXMTimeout1601	  )
	ON_WM_TIMER()
	ON_BN_CLICKED(IDC_BUTTON_STOP, &CDlg_t1601::OnBnClickedButtonStop)
END_MESSAGE_MAP()
...
void CDlg_t1601::InitCtrls()
{
	...
	// 초기값 세팅
	((CComboBox*)GetDlgItem( IDC_INBLOCK_GUBUN1 ))->SetCurSel( 1 );	// 주식금액수량구분1		1:수량,2금액
	((CComboBox*)GetDlgItem( IDC_INBLOCK_GUBUN2 ))->SetCurSel( 1 );	// 옵션금액수량구분2		1:수량,2금액  
	((CComboBox*)GetDlgItem( IDC_INBLOCK_GUBUN3 ))->SetCurSel( 0 );	// 선물금액수량구분4		1:수량,2금액         

	//-------------------------------------------------------------------------
	// OutBlock
	InitOutBlockCtrl( IDC_OUTBLOCK1, "코 스 피" );
	//InitOutBlockCtrl( IDC_OUTBLOCK2, "코 스 닥" );
	InitOutBlockCtrl( IDC_OUTBLOCK2, "선    물" );
	InitOutBlockCtrl( IDC_OUTBLOCK3, "콜 옵 션" );
	InitOutBlockCtrl( IDC_OUTBLOCK4, "풋 옵 션" );
}

void CDlg_t1601::InitOutBlockCtrl( UINT nID, LPCTSTR lpszTitle )
{
	InsertColumn4List( this, nID,  0, lpszTitle , TRUE , 10);
	InsertColumn4List( this, nID,  1, "개    인", FALSE, 10);
	InsertColumn4List( this, nID,  2, "외 국 인", FALSE, 10);
	InsertColumn4List( this, nID,  3, "기 관 계", FALSE, 10);
...
// 데이터 조회
//--------------------------------------------------------------------------------------
void CDlg_t1601::Request1601()
{
	//-----------------------------------------------------------
	// 현물정상주문(t1601) ( base21=SONAT000,headtype=A )
	...
	//-----------------------------------------------------------
	// 데이터 취득
    CString str_gubun1;	GetDlgItemText( IDC_INBLOCK_GUBUN1, str_gubun1 );		// 주식금액수량구분1
    CString str_gubun2;	GetDlgItemText( IDC_INBLOCK_GUBUN2, str_gubun2 );		// 옵션금액수량구분2
	CString str_gubun3 = "2";														// 사용안함(금액)  
    CString str_gubun4;	GetDlgItemText( IDC_INBLOCK_GUBUN3, str_gubun4 );		// 선물금액수량구분3  
	...
	SetPacketData( pckInBlock.gubun3, sizeof( pckInBlock.gubun3 ), str_gubun3, DATA_TYPE_STRING );		// 금액단위3     
...
// 데이터를 받음
//--------------------------------------------------------------------------------------
LRESULT CDlg_t1601::OnXMReceive1601( WPARAM wParam, LPARAM lParam )
{
	//-------------------------------------------------------------------------------------
	// Data를 받음
...
// Timeout 발생
//--------------------------------------------------------------------------------------
LRESULT CDlg_t1601::OnXMTimeout1601( WPARAM wParam, LPARAM lParam )
{
	g_iXingAPI.ReleaseRequestData( (int)lParam );
...
void CDlg_t1601::OnTimer(UINT_PTR nIDEvent)
{
...
void CDlg_t1601::OnBnClickedButtonStop()
{
	KillTimer(1);
