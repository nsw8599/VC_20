...
void CDlg_CFOAQ00600::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_INBLOCK_QRYSRTDT, m_sdate);
	DDX_Control(pDX, IDC_INBLOCK_QRYENDDT, m_edate);
	DDX_Control(pDX, IDC_INBLOCK_INPTPWD, m_pwd);
}
...
BEGIN_MESSAGE_MAP(CDlg_CFOAQ00600, CDialog)
	...
	ON_MESSAGE	 ( WM_USER + XM_RECEIVE_DATA, OnXMReceive600	  )
	ON_MESSAGE	 ( WM_USER + XM_TIMEOUT_DATA, OnXMTimeout600	  )
END_MESSAGE_MAP()
...
BOOL CDlg_CFOAQ00600::OnInitDialog() 
{
	...
	m_sdate.SetWindowTextA(strDate);
	m_edate.SetWindowTextA(strDate);
	m_pwd.SetWindowTextA("0716");
...
void CDlg_CFOAQ00600::InitCtrls()
{
	...
	((CComboBox*)GetDlgItem(IDC_INBLOCK_ACNTNO))->SetCurSel(1);	// 계좌번호         
	((CComboBox*)GetDlgItem(IDC_INBLOCK_PRDGRPCODE))->SetCurSel(0);	// 상품군코드 00.전체      
	((CComboBox*)GetDlgItem(IDC_INBLOCK_PRDTEXECTPCODE))->SetCurSel(0);	// 체결 0.전체     
	((CComboBox*)GetDlgItem(IDC_INBLOCK_STNLNSEQTP))->SetCurSel(0);	// 정렬구분 3.역순 4.정순 

	//-------------------------------------------------------------------------
	// OutBlock1
	nCol = 0;
	InsertColumn4List( this, IDC_OUTBLOCK1, nCol++, "레코드갯수      ", FALSE,    15 );    // [long  ,    5] 레코드갯수      
	InsertColumn4List( this, IDC_OUTBLOCK1, nCol++, "계좌번호        ", TRUE ,   15 );    // [string,   20] 계좌번호        
	InsertColumn4List( this, IDC_OUTBLOCK1, nCol++, "비밀번호		 ", TRUE ,    15 );    // [string,    8] 입력비밀번호    
	InsertColumn4List( this, IDC_OUTBLOCK1, nCol++, "조회시작일      ", TRUE ,    15 );    // [string,    8] 조회시작일      
	InsertColumn4List( this, IDC_OUTBLOCK1, nCol++, "조회종료일      ", TRUE ,    15 );    // [string,    8] 조회종료일      
	InsertColumn4List( this, IDC_OUTBLOCK1, nCol++, "옵션분류코드		", TRUE ,    15 );    // [string,    2] 선물옵션분류코드
	InsertColumn4List( this, IDC_OUTBLOCK1, nCol++, "상품군코드      ", TRUE ,    15 );    // [string,    2] 상품군코드      
	InsertColumn4List( this, IDC_OUTBLOCK1, nCol++, "체결구분        ", TRUE ,    15 );    // [string,    1] 체결구분        
	InsertColumn4List( this, IDC_OUTBLOCK1, nCol++, "정렬순서구분    ", TRUE ,    15 );    // [string,    1] 정렬순서구분    
	InsertColumn4List( this, IDC_OUTBLOCK1, nCol++, "통신매체코드    ", TRUE ,    15 );    // [string,    2] 통신매체코드    
	
	//-------------------------------------------------------------------------
	// OutBlock2
	nCol = 0;
	InsertColumn4List( this, IDC_OUTBLOCK2, nCol++, "레코드갯수  ", FALSE,   10 );    // [long  ,    5] 레코드갯수  
	...
	InsertColumn4List( this, IDC_OUTBLOCK3, nCol++, "주문시각            ", TRUE ,   15 );    // [string,    9] 주문시각            
	InsertColumn4List( this, IDC_OUTBLOCK3, nCol++, "선물옵션종목번호    ", TRUE ,   12 );    // [string,   12] 선물옵션종목번호    
	InsertColumn4List( this, IDC_OUTBLOCK3, nCol++, "종목명              ", TRUE ,   20 );    // [string,   40] 종목명              
	InsertColumn4List( this, IDC_OUTBLOCK3, nCol++, "매매구분            ", TRUE ,   10 );    // [string,   10] 매매구분            
	InsertColumn4List( this, IDC_OUTBLOCK3, nCol++, "정정취소구분", TRUE ,   10 );    // [string,   10] 정정취소구분명      
	InsertColumn4List( this, IDC_OUTBLOCK3, nCol++, "선물옵션호가유형코드", TRUE ,    10);    // [string,    2] 선물옵션호가유형코드
	InsertColumn4List( this, IDC_OUTBLOCK3, nCol++, "선물옵션호가유형명  ", TRUE ,   15 );    // [string,   40] 선물옵션호가유형명  
	InsertColumn4List( this, IDC_OUTBLOCK3, nCol++, "주문가              ", FALSE, 13.2 );    // [double, 13.2] 주문가              
	InsertColumn4List( this, IDC_OUTBLOCK3, nCol++, "주문수량            ", FALSE,   15 );    // [long  ,   16] 주문수량            
...
// 데이터 조회
//--------------------------------------------------------------------------------------
void CDlg_CFOAQ00600::Request600( BOOL bNext )
{
	//-----------------------------------------------------------
...
// 데이터를 받음
//--------------------------------------------------------------------------------------
LRESULT CDlg_CFOAQ00600::OnXMReceive600( WPARAM wParam, LPARAM lParam )
{
	//-------------------------------------------------------------------------------------
	// Data를 받음
...
// Timeout 발생
//--------------------------------------------------------------------------------------
LRESULT CDlg_CFOAQ00600::OnXMTimeout600( WPARAM wParam, LPARAM lParam )
{
	g_iXingAPI.ReleaseRequestData( (int)lParam );
