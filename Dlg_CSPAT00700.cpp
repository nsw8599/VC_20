...
{
	//-----------------------------------------------------------
// 현물정상주문(CSPAT00700) ( base21=SONAT000,headtype=B )
    ...
    CString strOrdPrc       ;		GetDlgItemText( IDC_INBLOCK_ORDPRC       , strOrdPrc        );		// [string,    2] 호가유형코드                    StartPos 71, Length 2
    CString strOrdprcPtnCode;		GetDlgItemText( IDC_INBLOCK_ORDPRCPTNCODE, strOrdprcPtnCode );		// [string,    1] 주문조건구분                    StartPos 73, Length 1
    CString strOrdCndiTpCode;		GetDlgItemText( IDC_INBLOCK_ORDCNDITPCODE, strOrdCndiTpCode ); 		// [double, 13.2] 주문가                          StartPos 74, Length 13
	...
	//-----------------------------------------------------------
	// 데이터 입력
	SetPacketData( pckInBlock.OrgOrdNo     , sizeof( pckInBlock.OrgOrdNo      ), strOrgOrdNo     , DATA_TYPE_STRING );		// 계좌번호                 
